...
**File:** `src/intelligence/prompts/local_llm_prompts_v2.py`  
**Type:** general_change  
**Detected at:** 2025-11-07T08:00:01.142293  
...
FINANCIAL_ANALYST_SYSTEM_V2 = """You are a senior financial analyst with 20 years of experienc
...
```
...
1. **Analyze the change:**
   - Read the full file: `src/intelligence/prompts/local_llm_prompts_v2.py`
...
```sql
-- PostgreSQL check
SELECT COUNT(*) FROM team_tools WHERE file_path LIKE '%src/intelligence/prompts/local_llm_prompts_v2.py%';
-- Should return > 0

-- Neo4j check
MATCH (n) WHERE n.file_path CONTAINS 'src/intelligence/prompts/local_llm_prompts_v2.py' RETURN count(n);
-- Should return > 0
```
...
curl -X POST http://localhost:6333/collections/destiny-team-framework-master/points/scroll \
  ...
  -d '{"filter": {"must": [{"key": "file_path", "match": {"text": "src/intelligence/prompts/local_llm_prompts_v2.py"}}]}}' | jq '.result.points | length'
# Should return > 0

# Redis check
docker exec kg-redis redis-cli KEYS "*local_llm_prompts_v2*"
# Should return > 0
```
...
**Task:** Propagate src/intelligence/prompts/local_llm_prompts_v2.py to databases  
