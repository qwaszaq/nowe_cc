...
**File:** `sql/realtime_updates/pg_20251105_104647_TODO_WEEK2.sql`  
**Type:** database_schema  
**Detected at:** 2025-11-05T11:00:00.523143  
...
1. **Analyze the change:**
   - Read the full file: `sql/realtime_updates/pg_20251105_104647_TODO_WEEK2.sql`
...
```sql
-- PostgreSQL check
SELECT COUNT(*) FROM team_tools WHERE file_path LIKE '%sql/realtime_updates/pg_20251105_104647_TODO_WEEK2.sql%';
-- Should return > 0

-- Neo4j check
MATCH (n) WHERE n.file_path CONTAINS 'sql/realtime_updates/pg_20251105_104647_TODO_WEEK2.sql' RETURN count(n);
-- Should return > 0
```
...
curl -X POST http://localhost:6333/collections/destiny-team-framework-master/points/scroll \
  ...
  -d '{"filter": {"must": [{"key": "file_path", "match": {"text": "sql/realtime_updates/pg_20251105_104647_TODO_WEEK2.sql"}}]}}' | jq '.result.points | length'
# Should return > 0

# Redis check
docker exec kg-redis redis-cli KEYS "*pg_20251105_104647_TODO_WEEK2*"
# Should return > 0
```
...
**Task:** Propagate sql/realtime_updates/pg_20251105_104647_TODO_WEEK2.sql to databases  
